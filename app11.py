...
def process_text_files(folder_path):
    ...
    return None
def convert_to_resume(transcription):
    doc = nlp(transcription)

    # Extracting Name (spaCy NER)
    name = re.search(r'hi my name is (\w+)', transcription)
    name = name.group(1) if name else "Not Available"

    # Extracting Education using regex
    education_pattern = r'pursuing (.*? in .+? at .+?)(?: I|$)'  # Captures education with institution
    education = re.search(education_pattern, transcription)
    education = education.group(1) if education else "Not Available"

    # Extracting Skills (manual search for common skills)
    skills = []
    skills_keywords = ["Python", "Machine Learning", "Jupyter", "numpy", "AWS", "SQL", "Java", "C++"]
    for skill in skills_keywords:
        if skill.lower() in transcription.lower():
            skills.append(skill)

    # Extracting Projects (using keyword-based approach)
    projects = []
    project_keywords = ["operating system scheduler", "fraud detection", "flight reservation system"]
    for project in project_keywords:
        if project.lower() in transcription.lower():
            projects.append(project)

    # Extracting Certifications (simple rule-based approach)
    certifications = []
    certifications_keywords = ["Amazon Web Services academic graduate"]
    for cert in certifications_keywords:
        if cert.lower() in transcription.lower():
    ...
    structured_data = {
        ...
        "education": education,
        "skills": skills if skills else ["Not Available"],
        "projects": projects if projects else ["Not Available"],
        "certifications": certifications if certifications else ["Not Available"]
    }
    ...
    # Add sections
    doc.add_heading('Resume: ' + structured_data['name'], 0)
    doc.add_heading('Education:', level=1)
    doc.add_paragraph(structured_data['education'])

    doc.add_heading('Skills:', level=1)
    doc.add_paragraph(", ".join(structured_data['skills']))
    ...
    doc.add_paragraph(structured_data['certifications'])

    # Save the document
    doc.save('structured_resume.docx')
...
def process_video(video_path, output_text_path):
    ...
    convert_to_resume(all_paragraphs)
